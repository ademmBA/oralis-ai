...
const StudentProfileSection = memo(({ userId, isDark }) => {
  const [data,    setData]    = useState({ level: '', major: '', enrollmentYear: '' });
  const [loading, setLoading] = useState(true);
  const [saving,  setSaving]  = useState(false);
  const [editing, setEditing] = useState(false);
  const [form,    setForm]    = useState({ level: '', major: '', enrollmentYear: '' });
  ...
  const selectCls = `w-full bg-transparent text-sm font-semibold outline-none border-b transition-colors capitalize ${
    isDark
      ? 'text-white border-gray-600 focus:border-red-400'
      : 'text-gray-900 border-gray-300 focus:border-red-400'
  }`;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const sp = json.studentProfile ?? {};
        const loaded = {
          level:          sp.level          ?? '',
          major:          sp.major          ?? '',
          enrollmentYear: sp.enrollmentYear?.toString() ?? '',
        };
        setData(loaded);
        setForm(loaded);
      } catch (err) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const sp = json.studentProfile ?? {};
      const saved = {
        level:          sp.level          ?? form.level,
        major:          sp.major          ?? form.major,
        enrollmentYear: sp.enrollmentYear?.toString() ?? form.enrollmentYear,
      };
      setData(saved);
      setEditing(false);
  ...
  return (
    <div className={`${cardCls} p-6 space-y-4`}>
      ...
      {/* Level — needs a select so built inline */}
      <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${isDark ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
        ...
        <div className="min-w-0 flex-1">
          <p className={`text-xs font-medium mb-0.5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Academic Level</p>
          {editing ? (
            <select name="level" value={form.level} onChange={handleChange} className={selectCls}>
            ...
            <p className={`text-sm font-semibold capitalize ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {data.level || '—'}
            </p>
          )}
        </div>
      ...
      {/* Major */}
      {editing
        ? <EditPill icon={BookOpen} label="Major / Field of Study" name="major"          value={form.major}          onChange={handleChange} isDark={isDark} />
        : <StatPill icon={BookOpen} label="Major / Field of Study" value={data.major}                               isDark={isDark} />
      }

      {/* Enrollment Year */}
      {editing
        ? <EditPill icon={Calendar} label="Enrollment Year"        name="enrollmentYear" value={form.enrollmentYear} onChange={handleChange} isDark={isDark} type="number" />
        : <StatPill icon={Calendar} label="Enrollment Year"        value={data.enrollmentYear}                      isDark={isDark} />
      }
...
StudentProfileSection.displayName = 'StudentProfileSection';
...
const ProfileSettings = memo(() => {
  ...
  const userId    = localStorage.getItem('userId');
  const role      = ROLE_META[profile.role] ?? ROLE_META.student;
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Account Info (toggles view/edit) + Academic Details side by side ── */}
      <div className="grid md:grid-cols-2 gap-5">
        ...
        {/* Academic Details — students only, sits right beside Account Info */}
        {profile.role === 'student' && (
          <StudentProfileSection userId={userId} isDark={isDark} />
        )}
...
const DisabilitiesSettings = memo(({
  disabilities, handleDisabilityToggle, isUpdatingDisabilities, updateMessage, handleDisabilitySubmit,
}) => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  return (
    <div className={`backdrop-blur-md rounded-xl p-6 border transition-colors duration-300 ${
      isDark ? 'bg-white/10 border-white/20' : 'bg-white/80 border-gray-200'
    }`}>
      <h3 className={`text-xl font-bold mb-4 flex items-center ${isDark ? 'text-white' : 'text-gray-900'}`}>
        <GraduationCap className="w-5 h-5 mr-2 text-blue-500" />Disabilities Settings
      </h3>
      <div className="space-y-6">
            ...
            return (
              <label key={disability}
                className={`flex items-center p-3 rounded-lg border cursor-pointer transition-all duration-200 ${
                  selected
                    ? isDark ? 'bg-blue-600/20 border-blue-500 text-blue-300' : 'bg-blue-100 border-blue-500 text-blue-700'
                    : isDark ? 'bg-gray-800/50 border-gray-600 text-gray-300 hover:bg-gray-700/50' : 'bg-gray-100 border-gray-300 text-gray-800 hover:bg-gray-200'
                }`}
              >
                <input type="checkbox" className="sr-only" checked={selected} onChange={() => handleDisabilityToggle(disability)} />
        ...
        <div className="flex justify-end">
          <button onClick={handleDisabilitySubmit} disabled={isUpdatingDisabilities}
        ...
        {updateMessage && (
          <div className={`flex items-center px-4 py-2 rounded-lg text-sm font-medium ${
            updateMessage.includes('successfully') ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'
          }`}>
            {updateMessage.includes('successfully') ? <Check className="w-4 h-4 mr-2" /> : <X className="w-4 h-4 mr-2" />}
            {updateMessage}
          </div>
        )}
      </div>
...
const StudentSettings = () => {
  ...
  const handlePasswordSubmit = useCallback(async (e) => {
    e.preventDefault();
    setPasswordMessage('');
    if (passwordForm.newPassword !== passwordForm.confirmPassword) { setPasswordMessage('Passwords do not match'); return; }
    if (passwordStrength < 3) { setPasswordMessage('Password is too weak'); return; }
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'appearance':    return <PlaceholderTab title="Appearance Settings" description="Customize the look and feel" />;
      case 'language':      return <PlaceholderTab title="Language Settings"   description="Change language and region"  />;

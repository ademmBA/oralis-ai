  ...
  return (
      <div className={`min-h-screen relative overflow-hidden ${isDark ? 'text-gray-200' : 'text-gray-800 light'}`}>
        <div className="relative z-10 max-w-6xl mx-auto px-4 py-12">
          ...
          <div className="mb-12">
            <h2 className={`text-2xl font-semibold mb-4 ${isDark ? 'text-red-400' : 'text-red-600'}`}>
              My Quizzes
            </h2>
            {quizzes.length > 0 ? (
                <ul className="space-y-4">
                  {quizzes.map((quiz) => (
                      <li
                          key={quiz.id}
                          className={`p-4 rounded-xl shadow border transition-all duration-300 hover:-translate-y-0.5 ${
                              isDark
                                  ? 'bg-gray-800/50 border-gray-700'
                                  : 'bg-gray-50 border-gray-200 hover:border-red-200 hover:shadow-red-50/80'
                          }`}
                      >

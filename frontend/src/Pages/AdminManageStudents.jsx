...
import {
  ...
  Edit, Trash2, UserX, UserCheck, Shield, Monitor,
...
const StudentDetail = ({ studentId, onBack, onEdit, onDeactivate, onActivate, onHardDelete, isDark, loadingId }) => {
...
const StudentManagement = () => {
  ...
  const handleEdit = (student) => { setCurrentStudent(student); setShowForm(true); setSelectedId(null); };
  const handleCancel = () => { setShowForm(false); setCurrentStudent(null); };

  const handleSubmit = async (studentData) => {
    try {
      const res = await axios.put(`${API}/students/${currentStudent.id}`, studentData, { headers: authHeaders() });
      setStudents(prev => prev.map(s => s.id === currentStudent.id ? { ...s, ...res.data } : s));
      setShowForm(false);
  ...
  const handleDeactivate = async (id) => {
    ...
    setLoadingId(id);
    try {
      await axios.delete(`${API}/students/${id}/deactivate`, { headers: authHeaders() });
  ...
  const handleActivate = async (id) => {
    setLoadingId(id);
    try {
      await axios.put(`${API}/students/${id}/activate`, {}, { headers: authHeaders() });
      setStudents(prev => prev.map(s => s.id === id ? { ...s, is_active: true } : s));
    } catch (e) { console.error(e); } finally { setLoadingId(null); }
  };

  const handleHardDelete = async (id) => {
    if (!window.confirm('Permanently delete this student? Cannot be undone.')) return;
    setLoadingId(id);
    try {
      await axios.delete(`${API}/students/${id}/delete`, { headers: authHeaders() });
  ...
  // ── Render: edit form ──
  if (showForm) return (
    <StudentForm onSubmit={handleSubmit} onCancel={handleCancel} mode="edit" initialData={currentStudent} />
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredStudents.length > 0 ? filteredStudents.map(student => (
          <div key={student.id} onClick={() => setSelectedId(student.id)}
            className={`group relative rounded-2xl p-5 border cursor-pointer transition-all duration-200 hover:-translate-y-1 hover:shadow-lg ${!student.is_active ? 'opacity-60' : ''} ${isDark ? 'bg-gray-900/40 border-gray-700 hover:border-red-400/50' : 'bg-white border-gray-200 hover:border-red-300 hover:shadow-red-100/50'}`}>
            <div className="flex items-center gap-3">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${isDark ? 'bg-gradient-to-br from-red-500 to-gray-600' : 'bg-gradient-to-br from-red-400 to-red-600'}`}>
                {student.profile_image ? <img src={student.profile_image} alt="" className="w-full h-full object-cover rounded-xl" /> : <User className="w-5 h-5 text-white" />}
              </div>
              <div className="min-w-0 flex-1">
            ...
            <div className="flex gap-1.5 mt-3">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${student.is_active ? (isDark ? 'bg-green-500/20 text-green-300' : 'bg-green-100 text-green-600') : (isDark ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-500')}`}>
                {student.is_active ? 'Active' : 'Inactive'}
              </span>
            </div>
            <div className="flex gap-1 mt-3 pt-3 border-t border-dashed border-gray-200/50" onClick={e => e.stopPropagation()}>
              <button onClick={() => handleEdit(student)} disabled={loadingId === student.id} title="Edit"
              ...
              {student.is_active
                ? <button onClick={() => handleDeactivate(student.id)} disabled={loadingId === student.id} title="Deactivate"
                ...
                : <button onClick={() => handleActivate(student.id)} disabled={loadingId === student.id} title="Activate"
              ...
              <button onClick={() => handleHardDelete(student.id)} disabled={loadingId === student.id} title="Delete permanently"
